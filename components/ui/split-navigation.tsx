    ...
    <>
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-3 w-full pointer-events-auto items-start">
          ...
          <div className="hidden md:flex flex-col gap-1.5 self-start justify-self-start w-fit p-3 bg-white/5 backdrop-blur-md border border-white/10 rounded-xl">
            {leftLinks.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className="group flex items-center text-[11px] font-normal uppercase tracking-[0.2em] text-[var(--color-pearl)] hover:text-[var(--color-gold)] transition-colors w-fit leading-none py-1 px-1"
              >
