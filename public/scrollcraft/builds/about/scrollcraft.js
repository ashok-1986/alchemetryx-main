...
(function (global) {
  ...
  function mount(root, opts) {
    ...
    function initPointer() {
      ...
      addEventListener('pointermove', pointerMoveHandler, { passive: true });
      ...
      (function pointerTick() {
        ...
        pointerTickHandle = requestAnimationFrame(pointerTick);
      })();
    }
    ...
    // Track prime listeners for cleanup
    var primeListeners = [
      { type: 'touchstart', handler: prime },
      { type: 'touchend', handler: prime },
      { type: 'pointerdown', handler: prime },
      { type: 'click', handler: prime },
      { type: 'scroll', handler: prime }
    ];

    if (typeof initPointer === 'function') {
      initPointer();
    }

    var fontReadyCleanup = null;
    if (document.fonts && document.fonts.ready) {
      // Line splitting measures line boxes, so it has to wait for the real face.
      fontReadyCleanup = document.fonts.ready.then(function () {
    ...
    layout();
    var tickHandle = requestAnimationFrame(tick);
    ...
    function destroy() {
      ...
      removeEventListener('focusin', focusHandler);
      removeEventListener('resize', resizeHandler);

      // Remove prime listeners
      for (var i = 0; i < primeListeners.length; i++) {
        removeEventListener(primeListeners[i].type, primeListeners[i].handler);
      }

      // Remove pointermove listener if it was added
      if (pointerMoveHandler) {
        removeEventListener('pointermove', pointerMoveHandler);
      }

      cancelAnimationFrame(tickHandle);
      cancelAnimationFrame(pointerTickHandle);

      // Remove from instances
      var idx = global.ScrollCraft.instances.indexOf(api);
      if (idx > -1) global.ScrollCraft.instances.splice(idx, 1);

      document.documentElement.classList.remove('sc-ready');
    }

    var api = { layout: layout, read: read, acts: acts, worlds: worlds, clips: playheads, lerp: LERP, destroy: destroy };
